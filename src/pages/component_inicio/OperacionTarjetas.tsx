...
const OperacionTarjetas = () => {
  ...
  const [numTarjetas, setNumTarjetas] =
    useState<{ Activo: boolean; numTarjeta: string }[]>();
  ...
  useEffect(() => {
    const url =
      "https://187.188.16.29:4431/webservice-app2/controllers/getTarjetasBancarias.php";
    let send = {
  ...
  const validarTarjeta = () => {
    ...
    if (datosTarjeta.current) {
      ...
      if (!luhnCheck(numTarjeta)) {
        ...
        const url =
          "https://187.188.16.29:4431/webservice-app2/controllers/addTarjeta.php";
        let data = {
          claveSocio: user?.Clav_Asociado,
  ...
  const confirmarToken = (numTarjeta: number) => {
    const url =
      "https://187.188.16.29:4431/webservice-app2/controllers/confirmarToken.php";
    let data = {
      claveSocio: user?.Clav_Asociado,
      numTarjeta: numTarjeta,
    };
      ...
      <div className="contenidoTarjetas">
        ...
        <IonRadioGroup value={tarjetaActiva}>
          {numTarjetas?.map((item, index) =>
            item.Activo ? (
              <IonRadio
                value={item.numTarjeta}
                labelPlacement="end"
                key={index}
              >
                {item.numTarjeta} (Activa)
              </IonRadio>
            ) : (
              <IonRadio
                value={item.numTarjeta}
        ...
        <IonContent className="altaTarjetas">
          ...
          <form ref={datosTarjeta}>
            <IonGrid>
              ...
              <IonRow>
                <IonCol>
                  <IonInput type="number" placeholder="MM" name="mes" />
                </IonCol>
                <IonCol>
                  <IonInput type="number" placeholder="YY" name="anio" />
                </IonCol>
              </IonRow>
